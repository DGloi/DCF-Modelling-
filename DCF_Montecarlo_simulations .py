...
def MonteCarloSim():
    ...
    sales_growth2 = np.random.normal(loc=0.1, scale=0.02, size=Repetition)
    sales_growth3 = np.random.normal(loc=0.1, scale=0.03, size=Repetition)

    ebitda_margin1 = np.random.normal(loc=0.14, scale=0.01, size=Repetition)
    ebitda_margin2 = np.random.normal(loc=0.14, scale=0.02, size=Repetition)
    ebitda_margin3 = np.random.normal(loc=0.14, scale=0.03, size=Repetition)

    depr_percent1 = np.random.normal(loc=0.032, scale=0.01, size=Repetition)
    depr_percent2 = np.random.normal(loc=0.032, scale=0.01, size=Repetition)
    depr_percent3 = np.random.normal(loc=0.032, scale=0.01, size=Repetition)

    nwc_percent1 = np.random.normal(loc=0.024, scale=0.01, size=Repetition)
    nwc_percent2 = np.random.normal(loc=0.024, scale=0.01, size=Repetition)
    nwc_percent3 = np.random.normal(loc=0.024, scale=0.01, size=Repetition)

    capex_percent1 = np.random.normal(loc=0.024, scale=0.01, size=Repetition)
    capex_percent2 = np.random.normal(loc=0.024, scale=.01, size=Repetition)
    capex_percent3 = np.random.normal(loc=0.024, scale=0.01, size=Repetition)
...
# WACC = %E * costE +%debt * cost of debt * (1 – corporate tax rate)
        ...
        output = pd.DataFrame([sales, ebitda, depreciation, ebit, nwc, change_in_nwc, capex, tax_payment, free_cash_flow], index=[
                          'Sales', 'EBITDA', 'depreciation', 'EBIT', "Net Working Capital", "Change in NWC", "CAPEX", "Tax", "Free Cash Flow"]).round(2)
    return output_distribution
plt.hist(MonteCarloSim(),bins=30, density=True,alpha=0.9, color="g")
plt.title("Monte Carlo simulation on the DCF model")
plt.show()
